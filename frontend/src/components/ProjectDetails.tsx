...
const ProjectDetailsInteractive: FC = () => {
  ...
  // --- Update handlers ---
  const handleTaskUpdate = async (index: number, key: keyof Task, value: any) => {
    try {
  ...
  const allTypes = Array.from(new Set(tasks.map((t) => t.type))).filter(Boolean);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "To Do": return "border-blue-400 text-blue-600 focus:ring-blue-500";
      case "In Progress": return "border-yellow-400 text-yellow-600 focus:ring-yellow-500";
      case "Done": return "border-green-400 text-green-600 focus:ring-green-500";
      default: return "border-gray-300 text-gray-900 dark:text-gray-100 focus:ring-blue-500";
    }
  };
      ...
      {tasks.length > 0 && (
        <div className="flex flex-wrap gap-2 my-6">
          ...
          <select value={assigneeFilter} onChange={(e) => setAssigneeFilter(e.target.value)} className="px-3 py-2 border rounded-lg">
            <option>All</option>
            {allAssignees.map((a) => <option key={a._id}>{a.name}</option>)}
          </select>
        </div>
      )}
          ...
          <table className="w-full text-left text-gray-900 dark:text-gray-100">
            ...
            <tbody>
              {filteredTasks.map((task, i) => (
                <tr key={task._id || i} className="border-t hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                  ...
                  <td className="py-3 px-4">
                    <select
                      value={task.priority}
                      onChange={(e) => handleTaskUpdate(i, "priority", e.target.value)}
                  ...
                  <td className="py-3 px-4">
                    <select
                      ...
                      className={`px-2 py-1 text-sm rounded-lg bg-white dark:bg-gray-800 font-medium focus:outline-none focus:ring-2 ${getStatusColor(task.status)}`}
                    >
                      <option value="To Do">To Do</option>
                      <option value="In Progress">In Progress</option>
                      <option value="Done">Done</option>
                  ...
                  <td className="py-3 px-4">
                    <select
                      ...
                      onChange={(e) => {
                        const user = orgMembers.find((u) => u._id === e.target.value);
                        if (user) handleTaskUpdate(i, "assignee", user);
                      }}
                      className="px-2 py-1 rounded-lg bg-white dark:bg-gray-800 text-sm focus:outline-none"
                    >
                      <option value="">—</option>
                      {orgMembers.map((a) => <option key={a._id} value={a._id}>{a.name}</option>)}
                    </select>
                  </td>
